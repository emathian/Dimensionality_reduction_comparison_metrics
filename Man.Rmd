...
* CP values' calcul are compueted in parallel according the list of k levels. Furthermore in order to compute graphics and statistics a inner join between data frames will be done according the samples' ID.
* Wilcoxon's tests assume that data are non independent.
* On the graphic data are scaled.
...
* **parallel** : optional boolean argument that have to be set to False to not use parallel computing.
...
A ggplot and wilcoxon's test results are returned.
...
This function allows to displays the plot of Moran Indexes for each variable and and each method, a scatter plot is generated if Moran Indexes are defined for only one k level otherwise boxplot are generated. 
