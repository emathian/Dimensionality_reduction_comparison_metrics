...
k3 <- knn2nb(knearneigh(pca_coords_xy, k=150, RANN=FALSE))
k3

r_meso_coords_xy <- as.matrix(R_meso_df[, 2:dim(R_meso_df)[2]])
r_meso_coords_xy <- apply(r_meso_coords_xy, 2, as.numeric)
k3 <- knn2nb(knearneigh(r_meso_coords_xy, k=150, RANN=FALSE))
k3

wm <- nb2mat(k3, style='B')
ww <- nb2listw(k3, style='B')
  ...
  for (i in 1:length(l_coords_data)){
    ...
    for (c_k in 1:length(listK)){
      ...
      for (j in 2:dim(spatial_data)[2]){
        if (dim(c_data)[2]>2){
          format_spatial_data = data.frame('Sample_ID' = sample_ID_vect ,'spa_att' = spatial_data[, j])
          MI = moran_index_HD(data = c_data, format_spatial_data, listK[c_k], merge = NULL)
        }
...
spa_VISTA <- data.frame("Sample_ID" = spatial_att$Sample_ID , "VISTA" = spatial_att$VISTA)
moran_index_HD(data= r_meso_df, spatial_att= spa_VISTA, K = 13, merge =TRUE)
...
spatial_cor_main(l_coords_data = list(PCA_coords_df, UMAP_coords_NN230, TM_coords_df,R_meso_df), spatial_data = spatial_att, listK = c(20,200), nsim = 101 )
